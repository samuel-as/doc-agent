...
no admin rights, no PATH changes, no registry. Node only enters the picture in the `npx`
install path below, and only to copy the files.
...
Installs once and is available in **every** project, as `/doc-agent:document` — the guides
are still written into the `docs/` folder of whichever project you are in. Each new release
published on `main` arrives on its own, at the next Claude Code session — no Node and no
`npx` involved. `/plugin update doc-agent@doc-agent` pulls one early;
`/plugin uninstall doc-agent@doc-agent` removes it.
...
| The skill does not show up after `/plugin install` | start a new Claude Code session — installing or updating a plugin only applies on restart |
...
skill folder so they travel with the bundle on a plugin or `npx skills add` install too
